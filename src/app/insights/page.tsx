...
export default function InsightsPage() {
  ...
  // 개선: 총검색량 상위 20개 + 문서수 적은 것 (문서수 0개 제외) - 최대 20개 노출
  const analyzeKeywordsForInsights = (keywords: any[], limit: number) => {
    const TARGET_COUNT = 20 // 최대 20개 노출
    const SEARCH_TOP_COUNT = 100 // 검색량 상위 100개에서 선정 (더 많은 풀 확보)
    ...
    // 1. 카페 잠재력: 전체 키워드에서 검색량 상위 100개 선정 → 카페 문서수 있는 것만 필터링 → 문서수 적은 순 정렬 → 최대 20개
    const cafeInsights = {
      title: "🔥 카페 잠재력 키워드",
      ...
      keywords: keywords
        ...
        .slice(0, SEARCH_TOP_COUNT) // 검색량 상위 100개 선정
        .filter(k => {
          const cafeDocs = k.cafe_total || 0
          return cafeDocs > 0 // 카페 문서수 0개 제외
        })
        .sort((a, b) => {
          // 문서수 적은 순 (1순위) + 검색량 높은 순 (2순위)
    ...
    // 2. 블로그 잠재력: 전체 키워드에서 검색량 상위 100개 선정 → 블로그 문서수 있는 것만 필터링 → 문서수 적은 순 정렬 → 최대 20개
    const blogInsights = {
      title: "📝 블로그 잠재력 키워드",
      ...
      keywords: keywords
        ...
        .filter(k => {
          ...
          return blogDocs > 0 // 블로그 문서수 0개 제외
        })
        .sort((a, b) => {
          // 문서수 적은 순 (1순위) + 검색량 높은 순 (2순위)
    ...
    // 3. 웹 잠재력: 전체 키워드에서 검색량 상위 100개 선정 → 웹 문서수 있는 것만 필터링 → 문서수 적은 순 정렬 → 최대 20개
    const webInsights = {
      title: "🌐 웹 잠재력 키워드",
    ...
    // 4. 뉴스 잠재력: 전체 키워드에서 검색량 상위 100개 선정 → 뉴스 문서수 있는 것만 필터링 → 문서수 적은 순 정렬 → 최대 20개
    const newsInsights = {
      title: "📰 뉴스 잠재력 키워드",
      ...
      keywords: keywords
        ...
        .filter(k => {
          ...
          return newsDocs > 0 // 뉴스 문서수 0개 제외
        })
        .sort((a, b) => {
          // 문서수 적은 순 (1순위) + 검색량 높은 순 (2순위)
    ...
    // 5. 광고 잠재력: 전체 키워드에서 검색량 상위 100개 선정 → 광고수 적은 것만 필터링 → 광고수 적은 순 정렬 → 최대 20개
    const adCountInsights = {
      ...
      keywords: keywords
        ...
        .filter(k => {
          ...
          return adCount < 5 // 광고수 5개 미만
        })
        .sort((a, b) => {
          // 광고수 적은 순 (1순위) + 검색량 높은 순 (2순위)
    ...
    // 6. 총문서 인사이트: 전체 키워드에서 검색량 상위 100개 선정 → 총 문서수 있는 것만 필터링 → 총 문서수 적은 순 정렬 → 최대 20개
